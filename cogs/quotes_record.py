...
class Recorder(commands.Cog):
    ...
    async def save_quote(self, ctx):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            if data:
                ...
                await ctx.send(f"⚠️ I already have that quote saved for **{ref_msg.author.display_name}**!")
                return
            
            await db.execute(
                "INSERT INTO quotes (guild_id, user_id, content, timestamp, channel_id) VALUES (?, ?, ?, ?, ?)",
                (ctx.guild.id, ref_msg.author.id, ref_msg.content, str(orig_time), orig_channel)
            )
            await db.commit()
    ...
    async def get_quote(self, ctx, member: discord.Member):
        ...
        try:
            async with aiosqlite.connect(self.db_path) as db:
                query = "SELECT content, timestamp, channel_id FROM quotes WHERE guild_id = ? AND user_id = ?"
            ...
            embed.add_field(name="Context", value=f"<#{channel_id}>\n<t:{timestamp_str}:R>", inline=False) 
